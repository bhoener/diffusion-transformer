...
class Autoencoder(nn.Module):
    def __init__(
        ...
        self.downsamples = nn.ModuleList(
            nn.Conv2d(latent_channels, latent_channels, 2, 2) for _ in range(num_downsamples)
        )
        ...
        self.upsamples = nn.ModuleList(
            nn.ConvTranspose2d(latent_channels, latent_channels, 2, 2) for _ in range(num_downsamples)
        )
        
        self.reconstruction_conv = nn.Conv2d(latent_channels, n_channels, 1, 1)
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        return latent, self.reconstruction_conv(x)
...
def main():
    ...
    src = Image.open("src/test_img.jpg").resize((img_size, img_size))
    ...
    plt.imshow(
        torch.cat((input_img.squeeze(0), pred.squeeze(0)), dim=1)
        .permute(1, 2, 0)
        .detach()
        .cpu()
        .numpy()
