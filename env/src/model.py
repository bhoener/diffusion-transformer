    ...
    def forward(
        ...
        for layer in self.multi_stream_layers:
            latent_x, text_x = layer(
                latent_x, text_x, multi_stream_condition, attn_mask=attn_mask.view(B, 1, 1, -1)
            )
        ...
        single_stream_attn_mask = torch.cat((torch.ones(B, N).to(latent.device), attn_mask), dim=-1)
        for i, layer in enumerate(self.single_stream_layers):
            x = layer(x, single_stream_condition, attn_mask=single_stream_attn_mask.view(B, 1, 1, -1))
            if i == repa_layer:
                repa_out = x[:, :N, :]
